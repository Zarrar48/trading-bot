        ...
        with placeholder.container():
            ...
            with col_l:
                ...
                if not df_trades.empty:
                   desired_cols = ['timestamp', 'side', 'price', 'quantity', 'reason']
        
                   actual_cols = [c for c in desired_cols if c in df_trades.columns]
        
                   display_df = df_trades[actual_cols].copy()
        
                   if 'side' in display_df.columns:
                      styled_df = display_df.style.map(
                         lambda x: 'color: #00E676' if x == 'BUY' else ('color: #FF5252' if x == 'SELL' else ''), 
                         subset=['side']
                      )
                    ...
                    st.info("No trades executed yet. Waiting for strategy triggers...")
