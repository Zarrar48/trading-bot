    ...
    try:
        ...
        if not insp.has_table("prices") or not insp.has_table("portfolio"):
            with placeholder.container():
                ...
                st.info("The database tables do not exist yet. Please run the `bot.py` script to initialize the database and start collecting data.")
        ...
        with engine.connect() as conn:
            ...
            if row_count == 0:
                with placeholder.container():
                    ...
                    st.caption("The bot is running but hasn't logged any candle data yet. Please wait for the next candle close.")
                time.sleep(5)
                continue
        ...
        with placeholder.container():
            ...
            fig.update_layout(template="plotly_dark", height=600, margin=dict(l=0, r=0, t=30, b=0), legend=dict(orientation="h", y=1.02))
